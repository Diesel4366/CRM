...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: order }, { data: settings }] = await Promise.all([
    supabase.from('orders').select(`
      *, client(name, phone, email),
      ...
      order_items(*),
      documents(doc_number, doc_type, issued_at, due_date)
    `).eq('id', id).single(),
  ...
  const invoiceDocs = ((order as any).documents ?? []).filter((d: any) => d.doc_type === 'invoice')
  const docNumber = invoiceDocs[0]?.doc_number ?? `СЧ-${(order as any).order_number}`
